...
from .folsom_dataset import FolsomDataset
...
def build_dataset(config, split='train'):
    ...
    common_args = {
        ...
        'cloud_threshold': config.get('cloud_threshold', 0.5),
    }

    augment = TemporalAugmentation() if split == 'train' else None

    if dataset_name == 'skippd':
    ...
    elif dataset_name == 'folsom':
        return FolsomDataset(
            root=config['root'],
            split=split,
            **common_args,
        )
